        ...
        {prompts && prompts.length > 0 ? (
          <div className="grid grid-cols-3 gap-2">
            {prompts.map((prompt, idx) => (
              <button
                key={idx}
                className="relative flex aspect-[1.3333] w-24 cursor-pointer flex-col items-start overflow-hidden rounded border border-gray-500 text-xs hover:bg-gray-100 dark:hover:bg-zinc-700"
                onClick={() => {
                  setPromptMessage(prompt.PROMPT);
                }}
              >
                <div className="flex w-full gap-1 bg-gray-200 px-1 dark:bg-zinc-100 dark:text-black">
