...
export const ServerDescription = ({
  ...
  return (
    <div className="rounded-lg bg-gray-50 p-4 dark:bg-zinc-700">
      ...
      {isLoading ? (
        <div className="h-6 w-3/4 animate-pulse rounded-lg bg-gray-400 dark:bg-zinc-600" />
      ) : toggleEdit ? (
        <div className="space-y-4">
          ...
          <button
            onClick={onSave}
            disabled={isUpdating}
            className="rounded-lg bg-gray-500 px-4 py-2 text-white hover:bg-gray-600 disabled:opacity-50 dark:bg-zinc-600 dark:hover:bg-zinc-700"
          >
            {isUpdating ? <ProgressCircle /> : '저장'}
          </button>
        </div>
      ) : (
        <p className="text-gray-900 dark:text-zinc-100">{comment}</p>
      )}
    </div>
  );
};
